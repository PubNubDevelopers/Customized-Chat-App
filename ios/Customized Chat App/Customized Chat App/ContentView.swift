...
struct ContentView: View {
    ...
    @State var typingIndicatorBuildShown: Bool = false
    ...
    var body: some View {
        VStack {
            ...
            Text("Typing Indicator Shown (Runtime): " + (typingIndicatorShown ? "Yes" : "No"))
            Text("Typing Indicator Shown (Build): " + (typingIndicatorBuildShown ? "Yes" : "No"))
        }
        .padding()
        .onAppear {self.testing();self.loadBuildConfig()}
    }
    ...
    func loadBuildConfig() {
        let buildConfig = Configuration.params
        if (buildConfig == "")
        {
            debugPrint("Configuration Not Specified")
            return
        ...
        do {
            let data = Data(buildConfig.utf8)
            debugPrint(data)
            if let json = try JSONSerialization.jsonObject(with: data, options: []) as? [String: Any] {
                if let configTypingIndicator = json["typing_indicator"] as? Int {
                    debugPrint(configTypingIndicator)
                    if (configTypingIndicator == 1) {
                        typingIndicatorBuildShown = true
                    }
                }
            }
        } catch let error as NSError {
