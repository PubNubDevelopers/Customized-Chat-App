...
export default function Home () {
  ...
  useEffect(() => {
    ...
    try {
      ...
      //  JSON Parse error
      console.log("Unable to parse configuration search param (expected base64 encoded JSON file)")
    }
  }, [encodedConfiguration])

  return (
      <main className='flex flex-col gap-8 items-center'>
        <div suppressHydrationWarning={true} className=''>Value of Typing Indicator (Runtime) {configTypingIndicator ? "Enabled" : "Disabled"}</div>
